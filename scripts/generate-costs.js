 ...
 * generate-costs.js
 *
 * Generates plain English COSTS.md from a PRD using Claude API
 *
 * Usage:
 *   node scripts/generate-costs.js <path-to-prd.md>
 *   node scripts/generate-costs.js jaws-analytics/PRD.md
 */
...
if (!prdPath) {
  ...
  console.error('Usage: node scripts/generate-costs.js <path-to-prd.md>');
  process.exit(1);
}
...
const outputPath = path.join(outputDir, 'COSTS.md');
const promptPath = path.join(__dirname, '..', 'prompts', 'owner-manual', 'costs-prompt.md');

console.log('=== COSTS.md Generator ===\n');
...
// Call Claude API
console.log('Calling Claude API to generate cost breakdown...');
console.log('');
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    try {
      ...
      console.log('✓ Generated COSTS.md successfully!');
      console.log('');
      ...
      console.log('');
      console.log('Next: Review the cost breakdown and verify accuracy against actual billing pages!');
