...
/**
 * generate-daily-operations.js
 *
 * Generates plain English DAILY-OPERATIONS.md from a PRD using Claude API
 *
 * Usage:
 *   node scripts/generate-daily-operations.js <path-to-prd.md>
 *   node scripts/generate-daily-operations.js jaws-analytics/PRD.md
 */
...
if (!prdPath) {
  ...
  console.error('Usage: node scripts/generate-daily-operations.js <path-to-prd.md>');
  process.exit(1);
}
...
const outputPath = path.join(outputDir, 'DAILY-OPERATIONS.md');
const promptPath = path.join(__dirname, '..', 'prompts', 'owner-manual', 'daily-operations-prompt.md');

console.log('=== DAILY-OPERATIONS.md Generator ===\n');
...
// Call Claude API
console.log('Calling Claude API to generate operations checklist...');
console.log('');
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    try {
      ...
      console.log('✓ Generated DAILY-OPERATIONS.md successfully!');
      console.log('');
      ...
      const checklistItems = (generatedContent.match(/- \[ \]/g) || []).length;
      console.log('Checklist items:', checklistItems);

      // Verify time estimates present
      const timeEstimates = (generatedContent.match(/⏱️|minutes|seconds/gi) || []).length;
      console.log('Time estimate indicators:', timeEstimates);

      console.log('');
      console.log('Next: Review that checklist items are actionable and have time estimates!');
