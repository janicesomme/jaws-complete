 ...
 *
 * Generates plain English WHEN-THINGS-BREAK.md from a PRD using Claude API
 *
 * Usage:
 *   node scripts/generate-troubleshooting.js <path-to-prd.md>
 *   node scripts/generate-troubleshooting.js jaws-analytics/PRD.md
 */
...
if (!prdPath) {
  ...
  console.error('Usage: node scripts/generate-troubleshooting.js <path-to-prd.md>');
  process.exit(1);
}
...
const outputPath = path.join(outputDir, 'WHEN-THINGS-BREAK.md');
const promptPath = path.join(__dirname, '..', 'prompts', 'owner-manual', 'when-things-break-prompt.md');

console.log('=== WHEN-THINGS-BREAK.md Generator ===\n');
...
// Call Claude API
console.log('Calling Claude API to generate troubleshooting guide...');
console.log('');
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    try {
      ...
      console.log('✓ Generated WHEN-THINGS-BREAK.md successfully!');
      console.log('');
      ...
      // Count common issues
      const issueCount = (generatedContent.match(/### Issue \d+:/g) || []).length;
      console.log('Troubleshooting issues:', issueCount);

      // Count "What This Looks Like" patterns
      const symptomsCount = (generatedContent.match(/\*\*What This Looks Like:\*\*/g) || []).length;
      console.log('Symptom descriptions:', symptomsCount);

      console.log('');
      console.log('Next: Review the guide and verify a non-technical person can follow the troubleshooting steps!');
