...
/**
 * generate-how-it-works.js
 *
 * Generates plain English HOW-IT-WORKS.md from a PRD using Claude API
 *
 * Usage:
 *   node scripts/generate-how-it-works.js <path-to-prd.md>
 *   node scripts/generate-how-it-works.js jaws-analytics/PRD.md
 */
...
if (!prdPath) {
  ...
  console.error('Usage: node scripts/generate-how-it-works.js <path-to-prd.md>');
  process.exit(1);
}
...
const outputPath = path.join(outputDir, 'HOW-IT-WORKS.md');
const promptPath = path.join(__dirname, '..', 'prompts', 'owner-manual', 'how-it-works-prompt.md');

console.log('=== HOW-IT-WORKS.md Generator ===\n');
...
// Call Claude API
console.log('Calling Claude API to generate step-by-step flow explanation...');
console.log('');
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    try {
      ...
      console.log('✓ Generated HOW-IT-WORKS.md successfully!');
      console.log('');
      ...
      // Count "When/Then" patterns
      const whenThenCount = (generatedContent.match(/\*\*When:\*\*/g) || []).length;
      console.log('When/Then steps:', whenThenCount);

      console.log('');
      console.log('Next: Read through the flow and verify a non-technical person can trace transactions mentally!');
